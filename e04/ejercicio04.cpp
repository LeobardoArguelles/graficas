...
int main(int argc, char **argv) {
	...
	GLFWwindow* window = glfwCreateWindow(W_WIDTH, W_HEIGHT, "Ejercicio 4", nullptr, nullptr);
	int* test = new int;
	...
	nVertices = (int)pow(3.0, divisions+1)*3;
	float vertices[nVertices];
	float colors[nVertices];

	// VALORES INICIALES
	vec3 a = vec3(-0.95f,  -0.95f, 0.0f);
	vec3 b = vec3(0.0f,   0.55f, 0.0f);
	vec3 c = vec3(0.95f,  -0.95f, 0.0f);

	divideTriangle(a, b, c, divisions, vertices, colors, 0, divisions);
...
void changeTriangle(data* d, int delta) {
	...
	d->vertices = vertices;
	d->colors = colors;
	divideTriangle(d->a, d->b, d->c, d->divisions, d->vertices, d->colors, 0, d->divisions);
